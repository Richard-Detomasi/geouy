...
#' @examples
#'\donttest{
#' x <- data.frame(dpto = "Montevideo", loc = "Montevideo",
#'                 dir = "Av. 18 de julio 1453")
#' puntos <- try(geocode_ide_uy(x), silent = TRUE)
...
geocode_ide_uy <- function(x, details = F) {
  ...
  # seq_len() y no 1:nrow(x): si todas las direcciones venian vacias, el filtro
  # de arriba deja cero filas y 1:0 recorre 1 y 0, que termina en "subindice
  ...
  for (i in seq_len(nrow(x))) {
    ...
    p <- NULL 
    # La espera es para espaciar los pedidos al servicio, asi que despues del
    # ultimo no hace falta: con una sola direccion la funcion esperaba diez
    # segundos antes de devolver, sin ningun pedido siguiente que espaciar.
    if (i < nrow(x)) Sys.sleep(10)
  }
  return(x)
}
