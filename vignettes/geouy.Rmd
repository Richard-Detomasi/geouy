...
```{r eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
# devtools::install_github("Richard-Detomasi/geouy", ref = 'master'); 
library(geouy)

# Simulamos una población
pop <- data.frame(x = sample(560000:585000,500),
                  y = sample(6136000:6160000,500))
# which_uy() trabaja con objetos sf, así que las coordenadas pasan a puntos
pop <- sf::st_as_sf(pop, coords = c("x", "y"), crs = 32721)
...
pop_loc[is.na(pop_loc$cat_loc_agr), "cat_loc_agr"] <- "Rural"

# y finalmente podemos hacer una tabla por estas regiones, por ejemplo: 
tabla_regiones <- sf::st_drop_geometry(pop_loc) %>%
        dplyr::group_by(cat_loc_agr) %>%
